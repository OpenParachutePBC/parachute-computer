...
- [x] `UserQuestion.fromJson` is called at most once per widget instantiation
- [x] Typing in the "Other" text field does not trigger JSON deserialization
- [x] Submit path uses the same cached list as initState for controller key lookups

## Work Log

| Date | Action | Learnings |
|------|--------|-----------|
| 2026-02-22 | Replaced `_parsedQuestions` getter with `late final List<UserQuestion> _questions` field initialized in `initState`. Renamed all references. | Bundled with todos 151, 154, 158, 165, 167, 170 in single commit. |
