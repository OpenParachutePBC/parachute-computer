...
- [x] After `answerQuestion()` succeeds and provider delivers `status = answered`, the card header shows "Answered" with green border
- [x] After server timeout delivers `status = timeout`, the card shows "Expired" with orange border regardless of local `_isAnswered` state
- [x] No visual divergence between local `_isAnswered` flag and the delivered `UserQuestionStatus`

## Work Log

| Date | Action | Learnings |
|------|--------|-----------|
| 2026-02-22 | Added `didUpdateWidget` override that resets `_isAnswered`, `_isSubmitting`, and `_errorMessage` when status transitions out of pending. | Bundled with todos 150, 154, 158, 165, 167, 170 in single commit. |
