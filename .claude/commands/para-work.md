...
2. **Consider Reviewer Agents** (Optional)
   ...
   - **python-reviewer**: Verify Python/FastAPI conventions (computer/ changes)
   - **flutter-reviewer**: Verify Flutter/Riverpod conventions (app/ changes)
   - **performance-oracle**: Check for performance issues
   - **security-sentinel**: Scan for security vulnerabilities
   - **parachute-conventions-reviewer**: Verify module boundaries and trust levels (cross-module changes)

   Run reviewers in parallel with Task tool:

   ```
   Task(code-simplicity-reviewer): "Review changes for simplicity"
   Task(python-reviewer): "Check Python conventions"
   ```
...
|------------------------|---------------------------|
| Plan has 3+ independent tasks across different modules | Plan is linear/sequential |
| Work spans both `computer/` and `app/` | Single module only |
| Multiple specialists needed (implement + test + review) | Single-focus work |
| Large feature with clear parallel tracks | Small feature or bug fix |

**Cost note:** Agent teams use ~4x more tokens than a single session. Each teammate gets a full context window.

### Enabling Agent Teams

Tell Claude to create a team and describe the structure you want:

> "Create an agent team to implement this plan. One teammate for the Python backend, one for the Flutter frontend, one running tests."

Or explicitly request: "Use agent team mode for this work"
