...
async def scan_migration(request: Request) -> dict[str, Any]:
    """
    Scan sessions to find those needing vault path migration.

    Returns a list of sessions that were created on a different vault root
    and may need migration to work properly on this machine.
    """
    orchestrator = get_orchestrator(request)
    return await orchestrator.scan_sessions_needing_migration()


class MigrateSessionRequest(BaseModel):
    """Request body for migrating a single session."""

    copy_transcript: bool = Field(
        True,
        alias="copyTranscript",
        description="Whether to copy the transcript file to the new location",
    )

    model_config = {"populate_by_name": True}


@router.post("/migration/session/{session_id}")
async def migrate_session(
    request: Request, session_id: str, body: Optional[MigrateSessionRequest] = None
) -> dict[str, Any]:
    """
    Migrate a single session to the current vault root.

    Updates the session's vault_root in the database and optionally copies
    the transcript file to the new location.
    """
    orchestrator = get_orchestrator(request)
    copy_transcript = body.copy_transcript if body else True
    return await orchestrator.migrate_session_vault_root(session_id, copy_transcript)


class MigrateAllRequest(BaseModel):
    """Request body for migrating all sessions."""

    copy_transcripts: bool = Field(
        True,
        alias="copyTranscripts",
        description="Whether to copy transcript files to new locations",
    )

    model_config = {"populate_by_name": True}


@router.post("/migration/all")
async def migrate_all_sessions(
    request: Request, body: Optional[MigrateAllRequest] = None
) -> dict[str, Any]:
    """
    Migrate all sessions that have a different vault root.

    This bulk operation updates all sessions to use the current vault root
    and optionally copies their transcript files.
    """
    orchestrator = get_orchestrator(request)
    copy_transcripts = body.copy_transcripts if body else True
    return await orchestrator.migrate_all_sessions(copy_transcripts)
